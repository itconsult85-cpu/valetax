            ...
            <div class="container-fluid">
                <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center gap-3 mb-4">
                    <div>
                        ...
                        <p class="text-secondary mb-0">Daftar pengguna yang sudah mengirim bukti dan ditandai selesai oleh bot.</p>
                    </div>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-sm-6 col-xl-3">
                        <div class="card border-0 shadow-sm h-100"><div class="card-body"><div class="text-secondary small">Total anggota</div><div class="fs-2 fw-semibold mt-1"><?= number_format($total, 0, ',', '.') ?></div><div class="text-success small mt-2"><i class="bi bi-check-circle me-1"></i>Status selesai + bukti tersimpan</div></div></div>
                    </div>
                    <div class="col-sm-6 col-xl-3">
                ...
                <div class="card border-0 shadow-sm">
                    ...
                    <div class="card-body p-0">
                        <div class="table-responsive">
                            <table class="table table-hover mb-0">
                                <thead class="table-light"><tr><th class="ps-4">No</th><th>Nama</th><th>Nomor Telepon</th><th>Progress</th><th>Screenshot</th><th>Mulai</th><th>Selesai</th><th class="pe-4">Aktif Terakhir</th></tr></thead>
                                <tbody>
                                <?php if ($users === []): ?>
                                    <tr><td colspan="8" class="text-center text-secondary py-5"><i class="bi bi-inbox fs-2 d-block mb-2"></i>Belum ada anggota yang ditandai selesai.</td></tr>
                                <?php else: ?>
                                    <?php foreach ($users as $index => $user): ?>
                                        <tr><td class="ps-4 text-secondary"><?= $index + 1 ?></td><td><div class="fw-semibold"><?= esc($user['user_name'] ?: 'Tanpa nama') ?></div><div class="text-secondary small"><?= esc($user['user_id'] ?: '-') ?></div></td><td><?= esc($user['phone_number'] ?: '-') ?></td><td><span class="badge text-bg-success"><i class="bi bi-check2 me-1"></i><?= (int) $user['current_step'] ?></span></td><td><?= (int) $user['screenshots_sent'] ?>/2</td><td><?= esc($user['started_at'] ?: '-') ?></td><td><?= esc($user['completed_at'] ?: '-') ?></td><td class="pe-4"><?= esc($user['last_active'] ?: '-') ?></td></tr>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                                </tbody>
                            </table>
                    ...
                    <div class="card-footer bg-white border-0 text-secondary small"><i class="bi bi-info-circle me-1"></i>Data menampilkan pengguna dengan <strong>completed_at</strong> terisi dan minimal satu bukti screenshot.</div>
                </div>
            </div>
        </div>
    </main>
